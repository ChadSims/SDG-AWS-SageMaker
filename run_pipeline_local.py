...
def tune_synthesiser(dataset, model):
...
def tune_synthesiser_parallel(dataset, model):
...
def train_synthesiser(dataset, model):
...
def tune_evaluators(dataset, model):
...
def tune_evaluators_parallel(dataset, model):
...
def eval_utility_synthetic(dataset, model):
...
def eval_fidelity(dataset, model):
...
def eval_privacy(dataset, model):
...
def eval_privacy_parallel(dataset, model):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, required=True, help='Name of the model to train: it can be "ctgan", "tvae", "copula_gan", "gaussian_copula", or "binary_diffusion"')
